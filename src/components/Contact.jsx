...
const Contact = () => {
  ...
  return (
    <div className="xl:mt-12 flex flex-col xl:flex-row gap-10 overflow-hidden">
      {/* Left column with heading + (on small) inline Earth + form */}
      <motion.div
        variants={slideIn("left", "tween", 0.2, 1)}
        className="flex-[0.75] bg-black-100 p-8 rounded-2xl"
      >
        {/* Heading + small-screen Earth side by side */}
        <div className="w-full">
          <div className="grid grid-cols-[1fr_auto] items-start gap-4">
            ...
            {/* Earth for sub-xl screens (appears to the right of heading) */}
            <div className="w-28 h-28 xs:w-32 xs:h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 xl:hidden">
              <EarthCanvas />
            </div>
          </div>
        </div>
        ...
        <form
          ...
          className="xl:mt-10 flex flex-col gap-8"
        >
      ...
      >
        {/* Only visible on xl+, so no duplicate visible simultaneously */}
        <EarthCanvas />
      </motion.div>
    </div>
  );
